...
```hcl
module "ecs-cluster" {
    source = "github.com/pbs/terraform-ecs-cluster-module?ref=x.y.z"
}
```
...
gh -R 'pbs/terraform-ecs-cluster-module' release download -p 'release.tar.gz' x.y.z
tar -xvf release.tar.gz -C 'terraform/modules/ecs-cluster'
rm -f release.tar.gz
```
...
```bash
git remote add -f ecs-cluster git@github.com:pbs/terraform-ecs-cluster-module.git
git subtree add --prefix terraform/modules/ecs-cluster ecs-cluster main --squash
```

If necessary, pin the repo version, rather than adding `main` as your subtree:

```bash
git subtree add --prefix terraform/modules/ecs-cluster ecs-cluster $REF --squash
```
